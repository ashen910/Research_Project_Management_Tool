...
const TopicRegDoc = require("../models/topicRegDoc");
...
router.post("/", upload.array('uploaded_Image', 10), async (req, res) => {
  try {
   ...
   for(let i=0;i<fileInArray.length;i++){
    ...
    let topicRegDoc = new TopicRegDoc({
      name: req.body.name,
      date:req.body.date,
      pdf : pdff.secure_url,
    ...
    await topicRegDoc.save();
    res.json(topicRegDoc);
  } catch (err) {
    console.log(err);
  }
});
...
router.get("/", async (req, res) => {
  try {
    let topicRegDoc = await TopicRegDoc.find();
    res.json(topicRegDoc);
  } catch (err) {
    console.log(err);
  }
...
router.delete("/:id", async(req,res)=>{
  ...
  try {    
    const id=req.params.id;
     let topicRegDoc = await TopicRegDoc.findByIdAndDelete(id);
     res.json(topicRegDoc);
  } catch (err) {
    console.log(err)    
  }
